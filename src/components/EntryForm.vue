...
<template>
  <form class="entry-form" @submit.prevent="$emit('submit')">
    <div class="amount-row">
      <input :value="input" class="amount-input" :class="entryDirection === -1 ? 'expense-input' : 'income-input'" type="text" inputmode="numeric" placeholder="Số tiền" aria-label="Số tiền" @input="$emit('update:input', $event.target.value)" />
